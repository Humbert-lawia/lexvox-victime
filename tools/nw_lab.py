...
def _norm(s: str) -> str:
    ...
    s = s.replace("’", "'").replace("ʼ", "'")  # apostrophes typographiques
...
def _count(term: str, text_norm: str) -> int:
    ...
    pluriel « s » toléré sur chaque mot — comportement observé du scoreur NW)."""
    words = _norm(term).split(" ")
    pat = (r"(?<![\w'])" + r"[\s '-]+".join(re.escape(w) + r"s?" for w in words)
           + r"(?![\w])")
