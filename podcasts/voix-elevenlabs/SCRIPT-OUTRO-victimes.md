# Script d'outro — chaîne VICTIMES

Texte lu par **Me Patrice Humbert**, avec sa voix clonée dans ElevenLabs.

**Cet outro ne dépend pas de l'épisode** : il ne contient ni le titre de
l'article ni le sujet du jour. Une seule génération ElevenLabs suffit donc
pour les 24 épisodes de la chaîne — enregistrez-la une fois, sous
`~/LEXVOX-PODCASTS/victimes/outro/outro-victimes.mp3`, et le montage la
réutilisera automatiquement.

```bash
python3 tools/voix_script.py --bloc outro --chaine victimes \
  --sortie ~/LEXVOX-PODCASTS/victimes/outro/outro-victimes.txt
```

Durée cible à la lecture : **25 à 35 secondes**.

⚠️ Ce texte est l'appel à l'action du cabinet. Il est verrouillé
déontologiquement (RIN art. 10.2 ; art. 10 loi n° 71-1130) : aucune promesse
de résultat, aucun montant, aucun titre non détenu. `voix_script.py` refuse
un script contenant une garantie de résultat. Toute modification passe par
une validation expresse — voir `PLAN-PODCASTS-2026-08.md` §2.4.

<<<SCRIPT
Maître Patrice Humbert à nouveau. Vous venez d'entendre l'essentiel, mais
chaque dossier a ses particularités, et c'est souvent là que se joue le
montant de l'indemnisation.

Pour faire valoir vos droits à la réparation intégrale de vos préjudices, ne
restez pas seul face aux assureurs. J'analyse votre dossier lors d'un premier
rendez-vous gratuit, dans nos bureaux d'Aix-en-Provence, de
Salon-de-Provence, d'Arles et de Marignane, ou partout en France en
visioconférence. Ensuite, tout est transparent : une convention d'honoraires
signée à l'avance, avec une part fixe et un complément au résultat.

Prenez rendez-vous sur le site du cabinet, ou appelez le zéro quatre, quatre-
vingt-dix, cinquante-quatre, cinquante-huit, dix. À bientôt.
SCRIPT>>>
